            ...
            (cond
                ...
                ((/= (setq index (isKeyWord word)) -1) ; Sets index's new value and controls if index is not -1(means that word is a keyword)
                    (progn
                      ...
                      (push (nth index KeywordsTokens) token-list)
                    )
                )
                ((/= (setq index (isOperator word)) -1) ; Sets index's new value and controls if index is not -1(means that word is a operator)
                    (progn
                      ...
                      (push (nth index OperatorTokens) token-list)
                    )
                )
...
(defun gpplexer (line)
  (setq token-l (list))
  (setq wordlist (split_line line)) ; Get list from split_line and set to wordlist
  (setq token-l (controlLine wordlist)) ; Call controlLine function and send list to function
  token-l
)